...
const Product = ({ item, ref }) => {
  ...
  return (
    <Link to={`/details/${item.slug}`} className="rounded-xl cursor-pointer hover:shadow-lg hover:px-1" ref={ref}>
      ...
      <div className="px-2 py-2">
        ...
        <p className="text-sm font-semibold">{item.formatted_price}</p>
      </div>
    </Link>
  );
};
