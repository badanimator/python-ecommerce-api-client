        ...
        <div className="bg-white rounded-3xl px-5 py-6 shadow-lg w-2/3 md:w-1/2 lg:w-auto">
          ...
          <form className=" border-t border-gray-200">
            ...
            {/* <ul role="list" className="px-2 py-3 font-medium text-gray-900">
              {subCategories.map((cat, key)=> (<li key={key}><a href="#" className="block px-2 py-3">{cat.name}</a></li>))}
              </ul> */}
                  ...
                  <div className="space-y-6 h-32 overflow-y-auto">
                    {cat.subfolder.map((folder, key)=>(
                      <div key={key} className="flex gap-3">
                        ...
                        <label htmlFor="filter-mobile-color-0" className="min-w-0 flex-1 text-gray-500">{folder.name}</label>
                      </div>
                    ))}
                  </div>
                </div>
